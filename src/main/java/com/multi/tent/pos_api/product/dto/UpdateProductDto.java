...
public class UpdateProductDto {
    ...
    @Schema(description = "Product name", example = "Coca Cola 600ml")
    private String name;

    @Size(max = 3000, message = "Description must not exceed 3000 characters")
    @Schema(description = "Product description", example = "Refreshing carbonated soft drink, new size")
    private String description;

    @NotNull(message = "Price is required")
    @DecimalMin(value = "0.01", message = "Price must be greater than 0")
    @Schema(description = "Product price", example = "30.00")
    private BigDecimal price;

    @NotNull(message = "Stock is required")
    @Min(value = 0, message = "Stock cannot be negative")
    @Schema(description = "Current stock quantity", example = "200")
